...
from converter.internal.length_type_value import parse_value, value_to_yaml, ParseHint
from converter.internal.multi_record_type import MultiRecordType
from converter.internal.errors import BinaryConversionError, YamlFormatError


class ManagementAccessRecordType(Enum):
...
def bound_check(start, end):
    def check(value):
        ...
        if not start <= length <= end:
            raise ValueError(
                f"Value '{value}' length {length} is out of range [{start}, {end}]"
            )
        return value
...
class ManagementAccessRecord(BasicRecord):
    ...
    def from_yaml(data: Any) -> "ManagementAccessRecord":
        ...
        try:
            return ManagementAccessRecord(
                ...
                data=VALUE_VALIDATORS[sub_type](
                    parse_value(
                        data[yaml_names.MANAGEMENT_ACCESS_RECORD_VALUE_KEY],
                        hint=(
                            ParseHint.ByteArray
                            if sub_type in {ManagementAccessRecordType.SystemUniqueID}
                            else None
                        ),
                    )
                ),
